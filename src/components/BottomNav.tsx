...
export function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-16 bg-card border-t border-border z-40 flex items-center justify-around md:hidden shadow-lg safe-bottom">
      {items.map((item) => {
        const isActive = path === item.to || (item.to === "/products" && path.startsWith("/products"));
        return (
          <Link
            key={item.label}
            to={item.to as any}
            className={`flex flex-col items-center justify-center w-full h-full gap-1 transition-all ${
              isActive ? "text-gold font-bold scale-105" : "text-muted-foreground hover:text-foreground"
            }`}
          >
